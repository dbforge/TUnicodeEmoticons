...
namespace TUnicodeEmoticons
{
    public static class Session
    {
        ...
        private static readonly ObservableCollection<string> _firstKeyCollection = new ObservableCollection<string> { "Control", "Alt" };
        private static readonly ObservableCollection<string> _secondKeyCollection = new ObservableCollection<string> { "Shift", "Win" };
        ...
        public static void InitializeData()
        {
            ...
            if (!File.Exists(FilePathProvider.TileDataFile))
                File.WriteAllText(FilePathProvider.TileDataFile, JsonConvert.SerializeObject(TUnicodeEmoticons.TileData.Default));
            if (!File.Exists(FilePathProvider.SettingsDataFile))
                File.WriteAllText(FilePathProvider.SettingsDataFile, JsonConvert.SerializeObject(Settings.Default), Encoding.Unicode);

            TileData = JsonConvert.DeserializeObject<TrulyObservableCollection<TileData>>(File.ReadAllText(FilePathProvider.TileDataFile));
        ...
        public static void InitializeHotKey()
        {
            ...
            if (keyData.HasFirstModifier)
                modifiers |= (KeyModifiers)Enum.Parse(typeof(KeyModifiers), _firstKeyCollection[keyData.FirstModifierIndex]);
            if (keyData.HasSecondModifier)
                modifiers |= (KeyModifiers)Enum.Parse(typeof(KeyModifiers), _secondKeyCollection[keyData.SecondModifierIndex]);

            _hotKey = new HotKey(Enum.GetValues(typeof(Key)).Cast<Key>().ToArray()[keyData.KeyIndex], modifiers, false)
            {
